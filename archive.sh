...
if [ $? -eq 0 ]; then
  echo "Step 2: Zipping the directory..."
  zip -r "$ZIP_FILE" "$REPO_NAME"
  
  # Check if zip was successful before deleting the source folder
  if [ $? -eq 0 ]; then
    echo "Step 3: Cleaning up..."
    rm -rf "$REPO_NAME"
    echo "Done! Successfully created $ZIP_FILE and removed the folder."
  else
