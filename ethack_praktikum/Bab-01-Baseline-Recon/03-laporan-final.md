...
## 1. Scope & Tujuan
- **Target**: 3000
- **Jaringan**: Host-only / internal bridge, terisolasi dari internet
- **Tujuan**: Latihan uji coba Nmap. baseline reconnaissance, dan pembiasaan dokumentasi
- **Tanggal**: 4 September 2026
- **Dilakukan oleh**: Hendra Manudinata

## 2. Ringkasan Eksekutif
Dilakukan pemindaian awal terhadap satu aplikasi latihan untuk memetakan layanan yang berjalan. Ditemukan satu layanan web HTTP aktif pada port 3000, yang bisa diakses secara bebas. Nmap melaporkan hanya port 3000 dengan protokol TCP. Namun, dengan pengecekan *service fingerprint* melaporkan bahwa target adalah **OWASP Juice Shop.**

## 3. Metodologi & Tools
| Tool       | Versi                        | Fungsi                  |
| ---------- | ---------------------------- | ----------------------- |
| Nmap       | 7.98 (`x86_64-pc-linux-gnu`) | Port & service scanning |
| Kali Linux | Kali Linux 2026.1 (`x86_64`) | Sistem operasi attacker |
Perintah yang dijalankan:

```
nmap -sV -p 3000 192.168.56.1

# Untuk mendapatkan informasi HTTP yang lebih detail:
nmap -sV --script=http-enum,http-headers -p 3000 192.168.56.1
```
## 4. Temuan

### Temuan #1 — Layanan HTTP Web Server Terbuka pada Port 3000
- **Deskripsi**:
Hasil pemindaian Nmap menunjukkan bahwa port **3000** (TCP) berada dalam status OPEN. Meskipun Nmap awal menandai service sebagai `ppp?`, service fingerprint menunjukkan balasan berupa header HTTP/1.1 200 OK yang menyajikan nama aplikasi web **OWASP Juice Shop**.
- **Bukti**:
```
# Service Fingerprint dari Nmap
<title>OWASP Juice Shop</title>
```

![](./02-screenshot/Pasted%20image%2020260904162121.png)

![](./02-screenshot/Pasted%20image%2020260904162449.png)
...
- **Skor CVSS**: *Versi tidak terdeteksi*
- **Rekomendasi**: Batasi akses port 3000 jika tidak ingin untuk diakses oleh semua interface.

## 5. Lampiran
- [x] Screenshot hasil `nmap`
- [x] Screenshot halaman target berhasil diakses
- [x] File log scan mentah (`01-nmap-scan-*.txt`)
